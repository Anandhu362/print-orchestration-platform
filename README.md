...
### Artifact 1: Live Factory Ledger (`ADSPRO WORK DETAILS - 2026`)
*Generated By:* `appendAdsproJobToSheet()` via Google Sheets API v4

![Live Factory Production Ledger](images/google%20sheets%20-%20Edited.png)

#### Data Integrity Verification:
- **Row 1**: `WADI MARKET JEBEL ALI 1` | `16X7 COUPON WITH NUMBERING` | Pages: `1` | Qty: `5000`
- **Row 2**: `ZAM ZAM KAWANEEJ 2` | `A3 TO A4` | Pages: `4` | Qty: `2000`
- **Row 3**: `NAGAMAT JEBEL ALI INDUSTRIAL AREA 1` | `23X33` | Pages: `8` | Qty: `6000`
- **Row 4**: `RUMAN AL QUOZ IND. AREA 1` | `A3 BOTHSIDE` | Pages: `2` | Qty: `2000`
- **Row 5**: `BEST HOME MUHAISNAH 2` | `A2 TO A3` | Pages: `4` | Qty: `5000`
- **Row 6**: `GO FRESH BURDUBAI` | `23X33` | Pages: `4` | Qty: `3000`
- **Row 7**: `RUMAN AL QUOZ IND AREA 1` | `23X33` | Pages: `4` | Qty: `3000`
...
- **[GATE 5] Multi-Probe Health Check**: Waited 30s boot stabilization; probed HTTP `/health` returning `{"status":"healthy","platform":"ADSPRO-Print-Automation","gitCommit":"4b8f767...","uptime":30}`.
...
flowchart TD
    subgraph IngressTier["1. Ingress Tier"]
        WA["WhatsApp Client (150MB+ PDFs & Specs)"]
    end

    subgraph EdgeGateway["2. Stateful Edge Gateway (Google Compute Engine - me-central1-a)"]
        WSS["Baileys TCP WebSocket\n(Persistent Session Keyring)"]
        AUTH["Zero-Trust Access Controller\n(JID / Group Whitelist)"]
        BUFFER["Sliding Burst Buffer\n(Orphan Matcher & Fast-Path)"]
    end

    subgraph ServerlessCompute["3. Serverless Compute (Google Cloud Run - me-central1)"]
        WORKER["Worker Microservice\n(2 vCPU / 4GB RAM)"]
        SANDBOX["Isolated PDF Sandbox\n(Poppler Render Engine)"]
        AI["Gemini 2.5 Flash\n(Multimodal Vision + Guardrails)"]
    end

    subgraph Persistence["4. Persistence & Production Sync"]
        RTDB[("Firebase Realtime DB\n(Session Auth)")]
        FS[("Firebase Firestore\n(User State & Audit Log)")]
        GCS[("Cloud Storage (me-central1)\n(14-Day Lifecycle Retention)")]
        SHEETS[("Google Sheets API\n(Live Factory Ledger)")]
    end

    WA -->|WSS Binary Packets| WSS
    WSS <-->|Hydrate / Save Keys| RTDB
    ...
    SANDBOX -->|Cover PNG + Caption| AI
    WORKER -->|Archive Asset| GCS
    AI -->|Row Append <4s| SHEETS
```
...
#### Scenarios Handled:
1. **Separated Message Sequence**: Customer uploads `RUMAN DEPARTMENT STORE.pdf` (57MB, 2 pages) at 7:58 PM. Text message arrives at 7:59 PM: *"A3 BOTHSIDE 2000 COPY WITH DIGITAL COPY"*. The `AdsproJobBuffer` absorbs the 1-minute delta and binds them seamlessly.
2. **Directly Captioned Payload**: Customer uploads `BEST HOME PROMO SEP.pdf` (41MB, 4 pages) with embedded caption: *"A2 TO A3 5000 COPY 4 PAGE"*. Expedited 60-second window triggers immediately.
3. **High-Payload Ingestion**: Customer uploads `NEW RUMAN .pdf` (120MB, 4 pages) followed by *"23/33 4 page 3000 COPY"*. Handled without V8 heap exhaustion via background streams.
...
sequenceDiagram
    ...
    VM->>Buffer: Register Document (< 1ms)
    alt Payload <= 150MB
        VM->>VM: Background Stream Download & Threaded Render
    else Payload > 150MB (Fast-Path)
        VM->>Buffer: Attach WhatsApp embedded jpegThumbnail
    end
    Customer->>VM: Sends WhatsApp Spec ("23/33 4 page 3000 COPY")
    ...
    Gemini-->>Worker: { shopName: "RUMAN AL QUOZ IND AREA 1", workDetails: "23X33", pageCount: 4, quantity: 3000 }
    Worker->>Storage: Async Archive Cover Image (gs://.../flyers/...)
    Worker->>Sheets: Append Row (SI No, Shop, Specs, Pages, Qty, VAT Formulas)
    Worker-->>Buffer: 200 OK { success: true, siNo: 7, rowIndex: 10 }
...
* **The Problem**: In the UAE commercial sector, many supermarkets share umbrella buying groups (e.g., "AL MADINA") while operating under unique store brands (e.g., "FURQAN", "RUMAN", "WADI"). Standard OCR frequently confuses the umbrella name with the store name, and misses the critical branch location (e.g., "DIP 2" vs. "Jebel Ali 1").
* **The Architectural Solution**: Domain-Specific AI Guardrails & Strict Schema Rules.
  - **Brand**: Extract the distinctive prefix brand; omit generic umbrella terms if a distinctive brand exists.
